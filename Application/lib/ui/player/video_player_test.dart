...
class _VideoPlayerState extends State<VideoPlayer1>
        ...
        body: RawKeyboardListener(
            ...
            if (event is RawKeyDownEvent &&
              ...
              if (!_visibile_controllers &&
                ...
                switch (rawKeyEventDataAndroid.keyCode) {
                  ...
                  case KEY_DOWN:
                    if (_visibile_subtitles_dialog) {
                      (_focused_subtitle == _subtitlesList.length - 1)
                          ? print("play sound")
                          : _focused_subtitle++;
                      ;
                    } else if (visibileSourcesDialog) {
                      (widget.focused_source ==
                              widget.sourcesListDialog.length - 1)
                          ? print("play sound")
                          : widget.focused_source++;
                      ;
                    } else {
                    ...
                    break;
                  case KEY_LEFT:
                    (_visibile_subtitles_dialog || visibileSourcesDialog)
                        ? print("play sound")
                        : (post_y == _video_controller_slider_position)
                            ? _fastForwardRewindVideo(-5)
                            : (post_x == 0)
                                ? print("play sound")
                                : post_x--;
                    break;
                    ...
                    (_visibile_subtitles_dialog || visibileSourcesDialog)
                        ...
                        : (post_y == _video_controller_slider_position)
                            ...
                            : ((post_y == _video_controller_play_position &&
                                        ...
                                        widget.next) ||
                                    (post_y ==
                                            _video_controller_play_position &&
                                        post_x == 3 &&
                                        !widget.next) ||
                                    (post_y ==
                                            _video_controller_settings_position &&
                                        post_x == 1))
                                ? print("play sound")
                                : post_x++;
                    break;
              ...
              setState(() {});
              if (_visibile_subtitles_dialog &&
                  _subtitlesScrollController != null) {
                _subtitlesScrollController.scrollTo(
                    index: _focused_subtitle,
                    alignment: 0.43,
          ...
          child: Stack(children: [
            ...
            if (_betterPlayerController != null)
              ui.VideoControllerWidget(
                ...
                video_controller_slider_position:
                    _video_controller_slider_position,
            ...
            ui.SubtitlesDialog(
                ...
                selected_subtitle: _selected_subtitle,
                focused_subtitle: _focused_subtitle,
                subtitlesScrollController: _subtitlesScrollController,
                visibile: _visibile_subtitles_dialog,
  ...
  Future<void> _fastForwardRewindVideo(int seconds) async {
    if (_betterPlayerController.videoPlayerController.value.duration != null &&
      ...
      int milli_second_seek_to = _betterPlayerController
              .videoPlayerController.value.position.inSeconds +
          seconds;
      if (milli_second_seek_to < 0) {
        milli_second_seek_to = 0;
      } else if (milli_second_seek_to >
          _betterPlayerController
              .videoPlayerController.value.duration.inSeconds) {
        milli_second_seek_to = _betterPlayerController
            .videoPlayerController.value.duration.inSeconds;
      }
  ...
  void FastRewind() {
    if (post_y == _video_controller_play_position && post_x == 2)
      _fastForwardRewindVideo(-10);
  }
  ...
  void FastForward() {
    if (post_y == _video_controller_play_position && post_x == 3)
      _fastForwardRewindVideo(10);
  }
  ...
  void _applySubtitle() {
    ...
    if (_selected_subtitle == 0) {
      ...
      subtitlesSource = BetterPlayerSubtitlesSource(
        ...
        urls: [_subtitlesList[_selected_subtitle].url],
      );
    }
  ...
  _goToNextEpisodePlayer() async {
    if (widget.sourcesListDialog[widget.selected_source].type == "youtube" ||
        widget.sourcesListDialog[widget.selected_source].type == "embed") {
      ...
      for (var i = 0; i < widget.sourcesListDialog.length; i++) {
        if (widget.sourcesListDialog[widget.selected_source].type !=
            "youtube") {
      ...
      Navigator.pushReplacement(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation1, animation2) => VideoPlayer1(
              sourcesList: _sources,
              selected_source: _new_selected_source,
              focused_source: _new_selected_source,
              poster: widget.poster,
              episode: widget.next_episode,
              season: widget.next_season,
  ...
  void _goToNextEpisode() {
    if (post_y == _video_controller_play_position && post_x == 4) {
      ...
      widget.focused_source = 0;
      widget.sourcesListDialog = widget
          .seasons[widget.next_season].episodes[widget.next_episode].sources;
